...
describe('GatekeeperService', () => {
  ...
  describe('Emergency Freeze Functionality (GENIUS Act Compliance)', () => {
    describe('Direct Freeze Operations (Regulatory)', () => {
      it('should freeze an address directly (regulatory admin)', async () => {
        ...
        await expect(service.freezeAddressDirect(testAddress)).resolves.not.toThrow();
      });

      it('should unfreeze an address directly (regulatory admin)', async () => {
        const testAddress = '0x742d35Cc6322C95582C305CdB2c1C75c559C7D57';
        
        await expect(service.unfreezeAddressDirect(testAddress)).resolves.not.toThrow();
      });
    });
    ...
    describe('Scoped Freeze Operations (Client Admin)', () => {
      it('should freeze an address within client scope', async () => {
        ...
        await expect(service.freezeAddressScoped(testAddress)).resolves.not.toThrow();
      });

      it('should unfreeze an address within client scope', async () => {
        const testAddress = '0x123456789abcdef123456789abcdef1234567890';
        
        await expect(service.unfreezeAddressScoped(testAddress)).resolves.not.toThrow();
      });
    });
  ...
  describe('🚨 CRITICAL INTEGRATION TEST: Emergency Freeze Transaction Prevention (GENIUS Act)', () => {
    it('should demonstrate that a frozen address successfully prevents token transactions', async () => {
      ...
      const initialSuspiciousFrozen = await service.isAddressFrozen(suspiciousAddress);
      const initialLegitimateFrozen = await service.isAddressFrozen(legitimateAddress);
      
      expect(initialSuspiciousFrozen).toBe(false);
      expect(initialLegitimateFrozen).toBe(false);
      
      console.log(`   ✅ Suspicious address frozen status: ${initialSuspiciousFrozen}`);
      console.log(`   ✅ Legitimate address frozen status: ${initialLegitimateFrozen}`);
      
      // Check initial transaction compliance
      const initialTransactionCheck = await service.isTransactionBlockedByFreeze(
        suspiciousAddress,
        legitimateAddress
      );
      ...
      console.log(`   ✅ Initial transaction blocked: ${initialTransactionCheck.isBlocked}`);
      ...
      console.log(`   🚨 EMERGENCY: Address ${suspiciousAddress} frozen by regulatory order`);
      ...
      const postFreezeSuspiciousStatus = await service.isAddressFrozen(suspiciousAddress);
      const postFreezeLegitimateStatus = await service.isAddressFrozen(legitimateAddress);
      ...
      console.log(`   ✅ Suspicious address is now frozen: ${postFreezeSuspiciousStatus}`);
      console.log(`   ✅ Legitimate address remains unfrozen: ${postFreezeLegitimateStatus}`);
      ...
      const blockedOutgoingTransaction = await service.isTransactionBlockedByFreeze(
        suspiciousAddress,  // Frozen sender
        legitimateAddress   // Unfrozen receiver
      );
      
      expect(blockedOutgoingTransaction.isBlocked).toBe(true);
      expect(blockedOutgoingTransaction.frozenAddresses).toContain(suspiciousAddress);
      expect(blockedOutgoingTransaction.reason).toBe('Sender address is frozen');
      
      console.log(`   🚫 Outgoing transaction blocked: ${blockedOutgoingTransaction.isBlocked}`);
      console.log(`   📝 Block reason: ${blockedOutgoingTransaction.reason}`);
      console.log(`   📋 Frozen addresses: [${blockedOutgoingTransaction.frozenAddresses.join(', ')}]`);
      
      // Test transaction TO frozen address (should be blocked)
      const blockedIncomingTransaction = await service.isTransactionBlockedByFreeze(
        legitimateAddress,  // Unfrozen sender
        suspiciousAddress   // Frozen receiver
      );
      
      expect(blockedIncomingTransaction.isBlocked).toBe(true);
      expect(blockedIncomingTransaction.frozenAddresses).toContain(suspiciousAddress);
      expect(blockedIncomingTransaction.reason).toBe('Receiver address is frozen');
      
      console.log(`   🚫 Incoming transaction blocked: ${blockedIncomingTransaction.isBlocked}`);
      ...
      console.log(`   ✅ Normal transaction allowed: ${!allowedTransaction.isBlocked}`);
      ...
      console.log(`   📊 Compliance check result: ${complianceWithFrozenSender.isCompliant}`);
      console.log(`   📝 Compliance reason: ${complianceWithFrozenSender.reason}`);
      
      // STEP 6: Clean up
      frozenCheckSpy.mockRestore();
      
      console.log(`\n✅ INTEGRATION TEST PASSED: Emergency freeze functionality successfully prevents transactions`);
      console.log(`✅ GENIUS Act Compliance Requirement MET: Platform can freeze addresses to comply with lawful orders`);
      console.log(`✅ Multi-tenant security verified: Proper permission separation implemented`);
    });
    ...
    it('should demonstrate scoped freeze functionality for client administrators', async () => {
      ...
      console.log(`   🔒 Client admin froze wallet in their organization scope`);
      ...
      console.log(`   ✅ Multi-tenant isolation maintained - clients can only freeze own wallets`);
      
      await service.unfreezeAddressScoped(clientWalletAddress);
      console.log(`   🔓 Client admin unfroze wallet in their organization scope`);
