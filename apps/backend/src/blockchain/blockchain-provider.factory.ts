...
export class BlockchainProviderFactory {
  ...
  private readonly registry = new Map<BlockchainProviderType, ProviderRegistryEntry>();
   ...
   */
  getCachedProviders(): Array<{ key: string; providerType: BlockchainProviderType; networkName: string }> {
   ...
   * @param networkName Network name
   */
  async removeProvider(providerType: BlockchainProviderType, networkName: string): Promise<void> {
  ...
  async clearCache(): Promise<void> {
    ...
    const disconnectPromises = Array.from(this.providers.values()).map(
      provider => provider.disconnect().catch(error => 
   ...
   */
  private validateConfiguration(config: ProviderConfiguration): ConfigValidationResult {
    const errors: string[] = [];
    ...
    if (config.providerType === BlockchainProviderType.HYPERLEDGER_FABRIC) {
      this.validateFabricConfiguration(config as FabricProviderConfiguration, errors);
    } else if ([
      ...
      this.validateEthereumConfiguration(config as EthereumProviderConfiguration, errors);
    } else {
   ...
   */
  private validateFabricConfiguration(config: FabricProviderConfiguration, errors: string[]): void {
   ...
   */
  private validateEthereumConfiguration(config: EthereumProviderConfiguration, errors: string[]): void {
  ...
  private createDefaultConfiguration(
    ...
    const baseConfig = {
      ...
      networkName,
      enabled: true,
      connectionTimeout: this.configService.get<number>('BLOCKCHAIN_CONNECTION_TIMEOUT', 30000),
      requestTimeout: this.configService.get<number>('BLOCKCHAIN_REQUEST_TIMEOUT', 10000),
      maxRetries: this.configService.get<number>('BLOCKCHAIN_MAX_RETRIES', 3),
      healthCheckInterval: this.configService.get<number>('BLOCKCHAIN_HEALTH_CHECK_INTERVAL', 60000),
    ...
    if (providerType === BlockchainProviderType.HYPERLEDGER_FABRIC) {
      return {
        ...
        providerType: BlockchainProviderType.HYPERLEDGER_FABRIC,
        fabricConfig: {
          connectionProfilePath: this.configService.get<string>('FABRIC_CONNECTION_PROFILE_PATH', ''),
          walletPath: this.configService.get<string>('FABRIC_WALLET_PATH', './wallet'),
          identityName: this.configService.get<string>('FABRIC_IDENTITY_NAME', 'appUser'),
          channelName: this.configService.get<string>('FABRIC_CHANNEL_NAME', networkName || 'kycchannel'),
          chaincodeName: this.configService.get<string>('FABRIC_CHAINCODE_NAME', 'kycattestation'),
          mspId: this.configService.get<string>('FABRIC_MSP_ID', 'Org1MSP'),
          enableDiscovery: this.configService.get<string>('FABRIC_ENABLE_DISCOVERY', 'true') === 'true',
          asLocalhost: this.configService.get<string>('FABRIC_AS_LOCALHOST', 'true') === 'true'
        }
